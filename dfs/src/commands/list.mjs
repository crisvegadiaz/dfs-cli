...
let text = (
  await import(`../locales/${language()}/list.json`, {
    with: { type: "json" },
  })
).default;
...
const formatDnfPackages = (output) => {
  const table = createTable(["Repository", "Version", "Package"], [4, 3, 3]);
  ...
  output.split("\n").forEach((line, idx) => {
    ...
    if (columns.length >= 3) {
      table.push([columns[0].trim(), columns[1].trim(), columns[2].trim()]);
    }
  });
...
const formatPacmanPackages = (output) => {
  const table = createTable(["Name", "Version"], [5, 5]);
  ...
  output.split("\n").forEach((line, idx) => {
    ...
    if (columns.length >= 2) {
      table.push([columns[0].trim(), columns[1].trim()]);
    }
  });
  console.log(table.toString());
};
